...
int fat16_get_root_entries(struct disk *disk,struct fat_private *private,struct fat_directory *directory){
     ...
     struct fat_directory_item *directory_item=kzalloc(sizeof(struct fat_directory_item));
...
int fat16_get_fat_entry(int cluster,int offset,struct fat_private *private){
    ...
    if(diskstreamer_seek(stream,starting_fat_table_pos)!=ALL_OK){
        res=-EIO;
        goto out;
    }
...
int fat16_get_cluster_for_offset(uint16_t cluster,int offset,struct fat_private *private){
    ...
    uint16_t clusters_ahead=offset/cluster_to_use;
    int res=0;
...
int fat16_read_internal_stream(struct disk_stream *stream,uint16_t cluster,int offset,int size,struct fat_private *private,void *item){
    ...
    size-=total_to_read;
    if(size>0){
        res=fat16_read_internal_stream(stream,cluster,size+total_to_read,size,private,item+total_to_read);
    }
out:return res;
...
struct fat_directory * fat16_load_directory(struct fat_directory_item *item, struct fat_private *private ){
    int res;
    ...
    directory->ending_sector_pos=directory_size/private->header.primary_header.BPB_bytes_per_sec;
    struct fat_directory_item *f_item=kzalloc(sizeof(struct fat_directory_item));
...
void* fat16_open(struct disk *disk, struct root_path *root,FILE_MODE mode){
    ...
    desc->item=fat16_get_directory_entry(disk,root->part);
    if(!desc->item){
        err_code=-EIO;
        goto err_out;
    }
...
int fat16_seek(void *desc,uint32_t offset,FILE_SEEK_MODE mode){
    ...
    switch(mode){
        case SEEK_SET:descriptor->pos=offset;
                      break;
        case SEEK_END:res=-EUNIMP;
                      break;
        case SEEK_CUR:descriptor->pos+=offset;
                      break;
        default      :res=-EINVARG;
                      break;
